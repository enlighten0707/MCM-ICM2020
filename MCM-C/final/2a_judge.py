...
scores = []
for i in range(length):
    scores.append(w[0]* (helpful_votes_[i]-ideal[0])* 
                  (helpful_votes_[i]-ideal[0])+ w[1] * 
                  (helpful_ratio_[i]-ideal[1])*(helpful_ratio_[i]-ideal[1])
                  + w[2]*(word_count_[i]-ideal[2])*(word_count_[i]-ideal[2]))
