...
ShowProvince=['Beijing','Shanghai','Shandong','Jiangsu','Guangdong','Fujian','Hebei','Henan','Jiangxi']
# Adjust the pupolation flow
Little_High_Province=['Hunan','Guangxi','Jilin','Liaoning']
...
#    if Province=='Guangxi' or Province=='Hunan':
#        print(Province)
#        print(alpha)
#        print(Place[2,52:66])
    China_Total_Pre+=Place
    if Province =='Hunan' or Province=='Guangxi':
        fig = plt.figure(4+i)
        plt.plot(range(len(prov.Confirmed[j])), prov.Confirmed[j], 'o-',label='Confirmed')
        plt.plot(range(len(prov.Confirmed[j])), Place[2,52:66], '+-',label='Prediction') #1.22->2.4
        plt.legend(loc = 'best')
...
#    if(prov.Province[j] in ShowProvince):
#        fig = plt.figure(4+j)
#        plt.plot(range(len(prov.Confirmed[j])), prov.Confirmed[j], 'o-',label='True Values')
