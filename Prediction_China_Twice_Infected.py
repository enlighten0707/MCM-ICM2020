...
fig = plt.figure()
plt.plot(range(46), China_Total_Pre[2,52:98], 'o-',label='Predictions') #1.22->2.20
...
Wuhan[: , 0]=[P, 0., 1., 0., 0.]
for i in range(119):
    Wuhan[0,i+1]=P-Wuhan[1,i]-Wuhan[2,i]-Wuhan[3,i]-Wuhan[4,i]+Wuhan[3,i]
    if i >= 67 and i <= 80:
        b -= 0.04                  # Infectious intensity decreases as temperature goes up
...
Hubei[: , 0]=[p, 0., 0., 0., 0.]
for i in range(119):
    Hubei[0,i+1]=p-Hubei[1,i]-Hubei[2,i]-Hubei[3,i]-Hubei[4,i]+Hubei[3,i]
...
for j in range(34):
    ...
    for i in range(119):
        Place[0,i+1]=p-Place[1,i]-Place[2,i]-Place[3,i]-Place[4,i]+Place[3,i]
        if i >= 67 and i <= 80:
            b -= 0.04   
...
#Date_List_tmp=['1.22','1.27','2.1','2.6','2.11','2.16','2.21','2.26','3.2','3.7']
plt.plot(range(46), China_Total_Pre[2,52:98], 'o-',label='Twice Infected') #1.22->2.20
plt.xticks(range(0,41,5),Date_List_tmp)
plt.xlabel('Date')
plt.ylabel('Confirmed Cases')
plt.legend(loc = 'best')
